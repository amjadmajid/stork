...
def decode_ROAccessReport(data):
    ...
    while True and len(data):
        try:
            ...
            logger.info('Unable to decode TagReportData')
            break
        ...
        if ret:
            ...
            logger.info("not all data was finished: %i"% len(data))
            break
...
def encode_AccessCommand (par):
    ...
    if(isinstance(par['OpSpecParameter'], dict)):
        ...
        for opspec in par['OpSpecParameter']:
...
def encode_C1G2Read (par):
    logger.debug('readOpspec = {}'.format(par))
...
# 16.2.1.2.1.1.1 C1G2TagInventoryMask Parameter
def encode_C1G2TagInventoryMask (par):
    msgtype = Message_struct['C1G2TagInventoryMask']['type']
    msg_header = '!HH'
    data = struct.pack('!B', (par['MB']and 3 or 0)<<6)
...
# 16.2.1.2.1.1.3 C1G2TagInventoryStateUnawareFilterAction Parameter
...
def decode_TagReportData(data):
    ...
    # grab opspec parameters
    par['OpSpecResult'] = {}
    while body:
        ret, body = decode_OpSpecResult(body)
        if ret:
            par['OpSpecResult'][ret['OpSpecID']] = ret
            if(ret['Result']):
                logger.info('BAD OpSpecResult={}'.format(ret))
        else:
            break
    osrcounter=0
    for os in par['OpSpecResult'].values():
        if(os['Result']):
            osrcounter+=1
    logger.debug('nr of good OpSpecResults = %i'%(len(par['OpSpecResult'])-osrcounter))
...
def decode_OpSpecResult (data):
    ...
    if msgtype == Message_struct['C1G2ReadOpSpecResult']['type']:
        ...
        par['ReadData'] = body[2:end].encode('hex')
...
class LLRPROSpec(dict):
    def __init__(self, llrpcli, msgid, priority=0, state='Disabled',
            ...
            session=2, tag_population=32, inventory_round_time = None):
        # Sanity checks
        if msgid <= 0:
        ...
        self['ROSpec'] = {
            ...
            'AISpec': {
                ...
                'AISpecStopTrigger': {
                    ...
                    'DurationTriggerValue': 75, # this value is overwritten by a user argument
                },
                'InventoryParameterSpec': {
            ...
            'ROReportSpec': {
                ...
                'N': 1, # this value is overwritten by a user argument
        ...
        if inventory_round_time is not None:
            logger.debug('inventory rounds of {}' \
                    ' ms'.format(inventory_round_time))
            self['ROSpec']['AISpec']['AISpecStopTrigger']['DurationTriggerValue'] = inventory_round_time
