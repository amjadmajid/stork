...
class LLRPMessage:
    ...
    def serialize (self):
        ...
        logger.debug('serializing command: %s', name)
...
class LLRPClient (LineReceiver):
    ...
    def __init__ (self, factory, duration=None, report_every_n_tags=None,
        ...
        self.inventory_round_time = inventory_round_time
    ...
    def handleMessage (self, lmsg):
        ...
        # ROSpec and advancing to state INVENTORYING.
            ...
            if msgName not in ('DELETE_ROSPEC_RESPONSE','DELETE_ACCESSSPEC_RESPONSE'):
    ...
    def rawDataReceived (self, data):
        logger.debug('got %d bytes from reader: ', len(data))#, data.encode('hex'))
    ...
    def send_DELETE_ACCESSSPEC (self, placeHolderArg, readSpecParam, writeSpecParam, stopParam,accessSpecID = 1, onCompletion=None):
        ...
        #d = defer.Deferred()
        #d.addCallback(self.startAccess, readSpecPar, writeSpecPar, stopSpecPar, accessSpecID)
        #d.addErrback(self.startAccess, readSpecPar, writeSpecPar, stopSpecPar, accessSpecID)
    ...
    def send_DELETE_ACCESSSPEC2 (self, accessSpecID, onCompletion =None):
        #logger.info('Deleting current accessSpec.')
        self.sendLLRPMessage(LLRPMessage(msgdict={
            'DELETE_ACCESSSPEC': {
                'Ver': 1,
...
##    ##    ##    ##     ## ##    ##     ##    ##     ## ##    ## ##    ## ##       ##    ## ##    ##
 ######     ##    ##     ## ##     ##    ##    ##     ##  ######   ######  ########  ######   ######
    ...
    # TODO: Fix this super ugly placeholder argument!
    def startAccess (self, readWords=None, writeWords=None, accessStopParam=None,
    target = None, accessSpecID = 1, *args):
        ...
        if not target:
            ...
            target = {
                ...
                'Pointer' : 0,
                'MaskBitCount': 0,#16*2,
                'TagMask': '',#'\x00\xFF\xFF\x00',#\xFF\xFF\xFF\xFF', #TODO make this useful
                'DataBitCount': 0,#16*2,
                'TagData': '',#'\xF1\x65\x34\x00',  # \xF1\x65\x34\x00\x00\x34\xB0\x07' # tag 269 needs 34000269
            }
        ...
        if readWords:
            ...
            for i in range(len(writeWords)):
                ...
                opSpecParam[i]['WriteDataWordCount'] = writeWords[i]['WriteDataWordCount']
    ...
    def startAccessSpec (self, _,opSpecs, accessSpecParams):
        #logger.info("startAccessSpec:{}".format(accessSpecParams))
        if 'target' in accessSpecParams:
            target = accessSpecParams['target']
        else:
            target = {
                'MB' : 1, # EPC = 1, userMem = 3
                'M': 1,
            ...
            logger.debug("Using default target")

        accessSpec = {
            'Type':  Message_struct['AccessSpec']['type'],
            'AccessSpecID': accessSpecParams['ID'],
            'AntennaID': 0, # all antennas
            'ProtocolID': AirProtocol['EPCGlobalClass1Gen2'],
            'C': False, # disabled by default
            'ROSpecID': 0, # all ROSpecs
            'AccessSpecStopTrigger': accessSpecParams['StopParam'],
            'AccessCommand': {
                'TagSpecParameter': {
                    'C1G2TargetTag': target,
                },
                'OpSpecParameter': opSpecs,
            },
            'AccessReportSpec': {
                'AccessReportTrigger': 1 #TODO  report at end of access
            }
        }

        d = defer.Deferred()
        d.addCallback(self.send_ENABLE_ACCESSSPEC, accessSpecParams['ID'])
        d.addErrback(self.panic, 'ADD_ACCESSSPEC failed')
    ...
    def nextAccessSpec(self, opSpecs, accessSpec):
        d = defer.Deferred()
        d.addCallback(self.startAccessSpec, opSpecs=opSpecs, accessSpecParams = accessSpec)
        d.addErrback(self.startAccessSpec, opSpecs=opSpecs, accessSpecParams = accessSpec)

        self.send_DELETE_ACCESSSPEC2(accessSpecID = accessSpec['ID'], onCompletion=d)
...
class LLRPClientFactory (ClientFactory):
    ...
    def nextAccess(self, readParam=None, writeParam=None, stopParam=None, accessSpecID = 1):
        ...
        for proto in self.protocols:
            proto.nextAccess(readSpecPar=readParam, writeSpecPar=writeParam, stopSpecPar=stopParam,accessSpecID=accessSpecID)

    def nextAccessSpec(self, opSpecs, accessSpec):
        #logger.info('Stopping current accessSpec.')
        for proto in self.protocols:
            proto.nextAccessSpec(opSpecs = opSpecs, accessSpec=accessSpec)

    def deleteAllAccessSpecs(self):

        for proto in self.protocols:
            proto.sendLLRPMessage(LLRPMessage(msgdict={
                'DELETE_ACCESSSPEC': {
                    'Ver': 1,
                    'Type': 41,
                    'ID': 0,
    ...
    def deleteAccessSpec(self,AccessSpecID):
