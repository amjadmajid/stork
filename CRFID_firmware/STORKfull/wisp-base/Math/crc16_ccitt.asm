...
r_numWords  .set	R14				;[] num of Words to calculate the CRC on if executing crc_words
...
; number of clock cycles incl RETA, excl JMP[5] to this function
; 20+7*R14(=nr_of_bytes)																									 *
;*************************************************************************************************************************************
...
crc16_a_byte:
    MOV.B   @r_dataPtr+,&CRCDIRB_L      ;[4] #3. add item to CRC checksum
...
;*************************************************************************************************************************************
;				unsigned int crc16_words(unsigned short preload,unsigned int *dataPtr, unsigned int numWords)						 *
; number of clock cycles incl RETA, excl JMP[5] to this function																	 *
; 20+7*R14(=nr_of_words)	    																									 *
;*************************************************************************************************************************************
crc16_words:                            ;[11] (2+2+2+(5))entry into function and setup with vals
    INV     r_crc                       ;[2] #1. bring CRC preload into working form (inverted) before operation
    MOV     r_crc,      &CRCINIRES      ;[1] #2. move CRC preload to correct register

crc16_a_word:
    MOV.W     @r_dataPtr+,  &CRCDIRB      ;[4] #3. add item to CRC checksum
