...
;	JLO     waitOnBits_5                                    ;[2]
;
; Pull CRC into R_scratch1.
;	MOV.B   (cmd+7), R_scratch1                             ;[3] bring in top 6 bits into b5-b0 of R_scratch1 (data.b15-b10)
;	MOV.B   (cmd+8), R_scratch2                             ;[3] bring in mid 8 bits into b7-b0 of R_scratch2 (data.b9-b2)
;	MOV.B   (cmd+9), R12                                    ;[3] bring in bot 2 bits into b7b6  of R12 (data.b1b0)
;
;	RLC.B   R_scratch2                                      ;[1]
;	RLC.B   R_scratch1                                      ;[1]
;	RLC.B   R_scratch2                                      ;[1]
...
;	BIS     R_scratch2, R_scratch1                          ;[1] merge b15-b8(R_scratch1) and b7-b(R_scratch2) together into R_scratch1
;
;	MOV 	R_scratch1, &(RWData.CRC)                   	;[3] store the received CRC
;
;	MOV     #(cmd), R_scratch2                              ;[2] load &cmd as dataPtr
;	MOV     #(7), R_scratch1                                ;[2] load num of bytes in BlockWrite
;	MOV     #(2), R_scratch0                                ;[2] load num of bits in BlockWrite

;	MOV     #ZERO_BIT_CRC, R12                              ;[2]
;	CALLA	#crc16Bits_ccitt								;[5+(20+7*nr_of_bytes+ x?*nr_of_bits)] = +/- [100]
;
...
; Prepare rfid transmission buffer, CRC16 0-bit and handle.
	...
	MOV     #ZERO_BIT_CRC, R12                              ;[2]
	CALLA   #crc16_ccitt                                    ;[5+(20+7*nr_of_bytes)] = [39]
...
call_my_BlockWriteCallback:					; call already before complete message is received
...
; Reset cmd buffer.
	MOV     #(cmd), R4                                      ;[?] Now reset cmd buffer. WHY USE R4?
	MOV     #(-3), R_bits                                   ;[3] Prepare to parse frame sync.
	CLR     R6                                              ;[1] Clear the bitcount.
...
checkEavesDropping:
	TST.B	&(rfid.isEavesDropping)
	JZ		exit_safely 	; Handle doesn't match AND I am not eavesdroping, so exit, without xcuting this blockwrite

; TCAL*0.85 - 2 us <= DELAY before response <= 20 ms
; call_my_BlockWriteCallback:
	CMP.B       #(0), &(RWData.bwrHook)                     ;[timing info not important for eavesdropping]
	JEQ         exit_safely 				                ; If there is no user callback, what is the point of eavesdropping?
